...
public class PostCreateViewModel : ViewModelBase<PostCreateRequest>
{
    ...
    public PostCreateViewModel(IPostReadOnlyRespository postReadOnlyRespository,
    ...
    public override async Task HandleAsync(PostCreateRequest request, CancellationToken cancellationToken)
    {
        try
        {
            var createPostResult =
                await _postReadWriteRespository.AddAsync(_mapper.Map<PostEntity>(request), cancellationToken);
