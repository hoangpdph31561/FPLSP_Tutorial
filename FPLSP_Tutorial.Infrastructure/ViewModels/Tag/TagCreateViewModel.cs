...
public class TagCreateViewModel : ViewModelBase<TagCreateRequest>
{
    ...
    public TagCreateViewModel(ITagReadWriteRepository tagReadWriteRepository, ILocalizationService localizationService,
        IMapper mapper)
    {
    ...
    public override async Task HandleAsync(TagCreateRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            var createResult =
                await _tagReadWriteRepository.AddTagAsync(_mapper.Map<TagEntity>(request), cancellationToken);
